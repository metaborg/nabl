...
public class NameResolution<S, L, R, O> implements INameResolution<S, L, R, O> {
    ...
    @Override public Set<IResolutionPath<S, L, O>> resolve(S scope) throws ResolutionException {
        return env(labelWF, Paths.empty(scope));
    }

    private Set<IResolutionPath<S, L, O>> env(LabelWF<L> re, IScopePath<S, L, O> path) throws ResolutionException {
        if(re.empty()) {
    ...
    //       the graph
    private Set<IResolutionPath<S, L, O>> env_L(Set<L> L, LabelWF<L> re, IScopePath<S, L, O> path)
            throws ResolutionException {
        final ImmutableSet.Builder<IResolutionPath<S, L, O>> envBuilder = ImmutableSet.builder();
        final Set<L> max_L = max(L);
        for(L l : max_L) {
            final Set<IResolutionPath<S, L, O>> env1 = env_L(smaller(L, l), re, path);
            envBuilder.addAll(env1);
            if(!dataEquiv.alwaysTrue() || env1.isEmpty()) {
                final Set<IResolutionPath<S, L, O>> env2 = env_l(l, re, path);
    ...
    private Set<IResolutionPath<S, L, O>> minus(Set<IResolutionPath<S, L, O>> env1,
            Set<IResolutionPath<S, L, O>> env2) {
        return env1.stream().filter(p1 -> env2.stream().noneMatch(p2 -> {
            return dataEquiv.eq(p1.getDeclaration(), p2.getDeclaration());
        })).collect(Collectors.toSet());
    }

    private Set<IResolutionPath<S, L, O>> env_l(L l, LabelWF<L> re, IScopePath<S, L, O> path)
            throws ResolutionException {
        return l.equals(scopeGraph.getEndOfPath()) ? env_EOP(re, path) : env_nonEOP(l, re, path);
    }

    private Set<IResolutionPath<S, L, O>> env_EOP(LabelWF<L> re, IScopePath<S, L, O> path) throws ResolutionException {
        ...
        ImmutableSet.Builder<IResolutionPath<S, L, O>> env = ImmutableSet.builder();
        for(O decl : scopeGraph.getData().get(path.getTarget(), relation)) {
            if(dataWF.wf(decl)) {
                env.add(Paths.resolve(path, decl));
            }
        }
        return env.build();
    }

    private Set<IResolutionPath<S, L, O>> env_nonEOP(L l, LabelWF<L> re, IScopePath<S, L, O> path)
            throws ResolutionException {
        if(!isEdgeComplete.test(path.getTarget(), l)) {
            throw new ResolutionException("Scope " + path.getTarget() + " is incomplete in edge " + l);
        }
        final ImmutableSet.Builder<IResolutionPath<S, L, O>> env = ImmutableSet.builder();
