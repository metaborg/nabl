...
import mb.nabl2.scopegraph.generalized.IGeneralizedNameResolution;
import mb.nabl2.scopegraph.generalized.IGeneralizedScopeGraph;
...
public class GeneralizedNameResolution<S extends IScope, L extends ILabel, O extends IOccurrence>
        implements IGeneralizedNameResolution<S, L, O> {

    private final IGeneralizedScopeGraph<S, L, O, ?> scopeGraph;
    ...
    public GeneralizedNameResolution(IResolutionParameters<L> params, IGeneralizedScopeGraph<S, L, O, ?> scopeGraph,
    ...
    @Override public boolean addAll(IGeneralizedNameResolution.ResolutionCache<S, L, O> cache) {
    ...
    private Optional<Set<IDeclPath<S, L, O>>> visibleEnv(S scope) {
        return env(io.usethesource.capsule.Set.Immutable.of(), order, wf, Paths.empty(scope), GeneralizedFilters.envFilter());
    }
    ...
    private Optional<Set<IResolutionPath<S, L, O>>> resolveEnv(io.usethesource.capsule.Set.Immutable<O> seenI, O ref) {
        ...
        if(scope.isPresent()) {
            return env(seenI.__insert(ref), order, wf, Paths.empty(scope.get()), GeneralizedFilters.resolutionFilter(ref));
        } else {
    ...
    private <P extends IPath<S, L, O>> Optional<Set<P>> env(io.usethesource.capsule.Set.Immutable<O> seenImports,
            IRelation<L> lt, IRegExpMatcher<L> re, IScopePath<S, L, O> path, GeneralizedFilters.Filter<S, L, O, P> filter) {
        if(re.isEmpty()) {
            return Optional.of(ImmutableSet.of());
        } else {
    ...
    private <P extends IPath<S, L, O>> Optional<Set<P>> env_l(io.usethesource.capsule.Set.Immutable<O> seenImports,
            ...
            GeneralizedFilters.Filter<S, L, O, P> filter) {
    ...
    private <P extends IPath<S, L, O>> Optional<Set<P>> env_L(Set<L> L,
            ...
            IScopePath<S, L, O> path, GeneralizedFilters.Filter<S, L, O, P> filter) {
    ...
    public static <S extends IScope, L extends ILabel, O extends IOccurrence> GeneralizedNameResolution<S, L, O>
            of(IResolutionParameters<L> params, IGeneralizedScopeGraph<S, L, O, ?> scopeGraph, Predicate2<S, L> isEdgeClosed) {
        return new GeneralizedNameResolution<>(params, scopeGraph, isEdgeClosed, Map.Transient.of(), Map.Transient.of(),
                Map.Transient.of());
    }

    public static class ResolutionCache<S extends IScope, L extends ILabel, O extends IOccurrence>
            implements IGeneralizedNameResolution.ResolutionCache<S, L, O>, Serializable {
