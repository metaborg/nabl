...
class BUEnv<S, L, D, P extends IResolutionPath<S, L, D>> {
    ...
    private BUPathSet.Immutable<S, L, D, P> currentPaths;
    private final BUPathSet.Transient<S, L, D, P> deprecatedPaths;

    private BUPathSet.Transient<S, L, D, P> addedPaths;
    private BUPathSet.Transient<S, L, D, P> removedPaths;
    ...
    BUEnv(BULabelOrder<L> compare, BUPathSet.Immutable<S, L, D, P> paths) {
        this.compare = compare;
        this.currentPaths = paths;
        this.deprecatedPaths = BUPathSet.Transient.of();
    }

    public BUPathSet.Immutable<S, L, D, P> pathSet() {
        return currentPaths;
    }

    public void apply(BUChanges<S, L, D, P> changes) throws InterruptedException {
        changes = changes.filter((key, p) -> !deprecatedPaths.paths(key).contains(p));
        final BUPathSet.Transient<S, L, D, P> paths = this.currentPaths.melt();
        final BUPathSet.Immutable<S, L, D, P> addPaths = changes.addedPaths();
        for(SpacedName name : addPaths.names()) {
            for(BUPathKey<L> key : addPaths.keys(name)) {
                addPaths(name, key, addPaths.paths(key), paths);
            }
        }
        final BUPathSet.Immutable<S, L, D, P> removePaths = changes.removedPaths();
    ...
    private void removePaths(SpacedName name, BUPathKey<L> key, Collection<P> oldPaths,
            BUPathSet.Transient<S, L, D, P> paths) throws InterruptedException {
        oldPaths = paths.remove(key, oldPaths);
        addedPaths.remove(key, oldPaths);
        removedPaths.addAll(key, oldPaths);
        deprecatedPaths.addAll(key, oldPaths);
    }
    ...
    private void addPaths(SpacedName name, BUPathKey<L> key, Collection<P> newPaths,
        ...
        for(BUPathKey<L> otherKey : paths.keys(name)) {
            ...
            if(result != null) {
                ...
                if(result < 0) {
                    ...
                    removedPaths.addAll(otherKey, otherPaths);
                    deprecatedPaths.addAll(key, otherPaths);
                }
                if(result > 0) {
        ...
        newPaths = paths.addAll(key, newPaths);
        addedPaths.addAll(key, newPaths);
    }

    public BUChanges<S, L, D, P> commitChanges() {
        BUChanges<S, L, D, P> changes = new BUChanges<>(addedPaths.freeze(), removedPaths.freeze());
        this.addedPaths = BUPathSet.Transient.of();
        this.removedPaths = BUPathSet.Transient.of();
        return changes;
    }
