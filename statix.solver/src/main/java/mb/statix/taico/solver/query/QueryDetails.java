...
public class QueryDetails<S extends D, L, D> implements IQueryDetails<S, L, D> {
    ...
    public QueryDetails(IConstraint constraint, TrackingNameResolution<S, L, D> nameResolution, List<ITerm> queryResult, Function<S, String> scopeOwner) {
        this(constraint, nameResolution.getTrackedEdges(), nameResolution.getTrackedData(), nameResolution.getDataWf(), queryResult, scopeOwner);
    }
    
    public QueryDetails(IConstraint constraint, Multimap<S, LabelWF<L>> edges, Multimap<S, LabelWF<L>> data, DataWF<D> dataWf, List<ITerm> queryResult, Function<S, String> scopeOwner) {
    ...
    private Set<String> computeModules() {
