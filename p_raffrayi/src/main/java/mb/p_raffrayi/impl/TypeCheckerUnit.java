...
class TypeCheckerUnit<S, L, D, R> extends AbstractUnit<S, L, D, R> implements IIncrementalTypeCheckerContext<S, L, D, R> {
    ...
    @Override public <Q> IFuture<R> runIncremental(Function1<Boolean, IFuture<Q>> runLocalTypeChecker,
        ...
        // @formatter:off
        final Set<IFuture<Tuple2<Boolean, io.usethesource.capsule.Map.Immutable<S, S>>>> confirmations = previousResult
            .queries()
            .stream()
            .map(q -> {
        ...
        new AggregateFuture<>(confirmations).whenComplete((r, ex) -> {
            if(r.stream().allMatch(Tuple2::_1)) {
                ...
                final io.usethesource.capsule.Map.Transient<S, S> patches = CapsuleUtil.transientMap();
                ...
                for(S oldScope : Sets.union(edgeEntries.keySet(), dataEntries.keySet())) {
                    ...
                    if(context.owner(oldScope) != self) {
                        ...
                        datum.ifPresent(d -> setDatum(scope, scopeImpl.substituteScopes(d, patches)));
                    }
