...
public class set_fixed_point_0_1 extends Strategy {
    ...
    @Override public IStrategoTerm invoke(final Context context, final IStrategoTerm current,
        ...
        if(!Tools.isTermList(base)) {
            throw new java.lang.IllegalArgumentException("Expected base set, got " + base);
        }
        final Set.Immutable<IStrategoTerm> baseSet = CapsuleUtil.toSet(Arrays.asList(base.getAllSubterms()));
        ...
        for(IStrategoTerm varEq : current.getAllSubterms()) {
            if(!Tools.isTermTuple(varEq) || varEq.getSubtermCount() != 2) {
                throw new java.lang.IllegalArgumentException("Expected tuple of variable and components, got " + varEq);
            }
            final IStrategoTerm var = varEq.getSubterm(0);
            final IStrategoTerm eq = varEq.getSubterm(1);
            eqs.put(var, parse(eq));
        ...
        while(again) {
            again = false;
            for(IStrategoTerm var : eqs.keySet()) {
                final Set.Immutable<IStrategoTerm> varResult = eqs.get(var).apply(values);
                final Set.Immutable<IStrategoTerm> prevResult = values.put(var, varResult);
    ...
    private Eq parse(IStrategoTerm term) {
        ...
        if(Tools.hasConstructor((IStrategoAppl) term, "Union", 1)) {
            ...
            return new Union(Streams.stream(components).map(this::parse).collect(ImmutableList.toImmutableList()));
            ...
            return new Intersection(
                    Streams.stream(components).map(this::parse).collect(ImmutableList.toImmutableList()));
        } else {
            return new Var(term);
        }
    }
