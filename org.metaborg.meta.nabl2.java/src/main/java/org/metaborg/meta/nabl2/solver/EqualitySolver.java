...
public class EqualitySolver implements ISolverComponent<IEqualityConstraint> {
    ...
    private ITerm findShallow(ITerm term) {
        return M.var(this::findVarRep).match(term).orElse(term);
    }
        ...
        return leftRep.matchOrThrow(Terms.checkedCases(
                ...
                applLeft -> CM.cases(CM.appl(applRight -> {
                    if(!(applLeft.getOp().equals(applRight.getOp()) && applLeft.getArity() == applRight.getArity())) {
                        throw new UnsatisfiableException("Cannot unify different term constructors.");
                    }
                    unifys(applLeft.getArgs(), applRight.getArgs());
                    return unit;
                }), CM.var(varRight -> unify(varRight, applLeft))).matchOrThrow(rightRep)
                        .orElseThrow(() -> new UnsatisfiableException()),
                listLeft -> CM.cases(CM.list(listRight -> {
                    if(listLeft.getLength() != listRight.getLength()) {
                        throw new UnsatisfiableException("Cannot unify lists of different length.");
                    }
                    unifys(listLeft, listRight);
                    return unit;
                }), CM.var(varRight -> unify(varRight, listLeft))).matchOrThrow(rightRep)
                        .orElseThrow(() -> new UnsatisfiableException()),
                stringLeft -> CM.cases(CM.string(stringRight -> {
                    if(!stringLeft.getValue().equals(stringRight.getValue())) {
                        throw new UnsatisfiableException("Cannot unify different strings.");
                    }
                    return unit;
                }), CM.var(varRight -> unify(varRight, stringLeft))).matchOrThrow(rightRep)
                        .orElseThrow(() -> new UnsatisfiableException()),

                integerLeft -> CM.cases(CM.integer(integerRight -> {
                    if(integerLeft.getValue() != integerRight.getValue()) {
                        throw new UnsatisfiableException("Cannot unify different integers.");
                    }
                    return unit;
                }), CM.var(varRight -> unify(varRight, integerLeft))).matchOrThrow(rightRep)
                        .orElseThrow(() -> new UnsatisfiableException()),
                varLeft -> M.cases(M.var(varRight -> {
