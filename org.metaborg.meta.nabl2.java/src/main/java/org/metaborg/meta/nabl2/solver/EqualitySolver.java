...
public class EqualitySolver implements ISolverComponent<IEqualityConstraint> {
    ...
    @Override public void finish() throws UnsatisfiableException {
        for (IEqualityConstraint constraint : defered) {
            throw new UnsatisfiableException("Unexpected unsolved equality.", constraint);
        }
    }
    ...
    public Unit unify(ITerm left, ITerm right) throws UnsatisfiableException {
        ...
        return leftRep.matchOrThrow(Terms.checkedCases(
            ...
            applLeft -> CM.cases(
                        ...
                        ).matchOrThrow(rightRep).orElseThrow(() -> new UnsatisfiableException()),
            listLeft -> CM.cases(
                            CM.list(listRight -> {
    ...
    private boolean canUnify(ITerm left, ITerm right) {
        return left.match(Terms.<Boolean> cases(
            // @formatter:off
            applLeft -> M.cases(
                            M.appl(applRight -> (applLeft.getOp().equals(applRight.getOp()) && (applLeft.getArity() == applLeft.getArity()) && canUnifys(applLeft.getArgs(), applRight.getArgs()))),
                            M.var(varRight -> true)
