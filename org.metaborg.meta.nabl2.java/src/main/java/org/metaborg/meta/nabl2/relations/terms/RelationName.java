...
public abstract class RelationName extends AbstractApplTerm implements IRelationName, IApplTerm {
    ...
    public static IMatcher<RelationName> matcher() {
        return M.preserveAttachments(M.cases(
            // @formatter:off
            M.appl0(OP0, (t) -> ImmutableRelationName.of(Optional.empty())),
            M.appl1(OP1, M.stringValue(), (t, ns) -> ImmutableRelationName.of(Optional.of(ns)))
            // @formatter:on
        ));
    }

    @Override
    protected RelationName check() {
        return this;
    }
